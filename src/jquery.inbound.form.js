    ...
    function str2slug(str) {
      ...
      // Buider Form
        _BuiderForm: function (fields, token, secret, modal) {
          endform += "<input id='token' value='"+token+"' name='token' type='hidden'>" + "<input id='secret' value='"+secret+"' name='secret' type='hidden'>" + "</form>";
          // if has radio field
          if (fields.radio) {
            for (var i in fields.radio) {
              var value = fields.radio[i];
              value = str2slug(value);
              radios += "<label class='radio-inline' for='level-" + i + "'><input name='level' id='level-" + i + "' value='" + value + "' type='radio'>" + fields.radio[i] + "</label>";
            }
          }
          var radiofield = "<div class='form-group'> " + "<label for='level'>Level</label> " + "<div class=''> ";
          ...
          var selectfield = "<div class='form-group'>" + "<label for='state'>State</label> " + "<select id='state' name='state' class='form-control'>";
          selectfield = selectfield + options;
          selectfield += "</select>" + "</div>";

          if (selectfield && radiofield) {
            form = initform + selectfield + radiofield + endform;
          } else {
            if ( selectfield  ) {
              form = initform + selectfield + endform;
            } else if (radiofield  ) {
              form = initform + radiofield + endform;
            } else {
